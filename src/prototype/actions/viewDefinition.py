...
def addProtoFiedToList(fieldList,  pEntity, fieldBase, zoomName):
    ...
    for pProperty in pEntity.property_set.all():
        ...
        if len(fieldBase) > 0:
            ...
            fkFieldList = []
            addProtoFiedToList(
                fkFieldList, zoomEntity, fName, slugify2(zoomEntity.code))
    ...
    if len(fieldBase) == 0:
        for fName in ['smOwningUser', 'smOwningTeam', 'smCreatedBy', 'smModifiedBy', 'smWflowStatus', 'smRegStatus', 'smCreatedOn', 'smModifiedOn', 'smUUID', 'smVersion']:
